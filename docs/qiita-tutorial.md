...
**[small OS](https://github.com/rex0220/small-os)** は TypeScript + Vite で実装した、**ブラウザー上で動作する OS シミュレーター**です。  
...
commands.set('istat', async (args, _stdin, sys) => {
  ...
  return [
    ...
    `size:     ${stat.size}`,
    `links:    ${stat.nlink}`,
    `created:  ${new Date(stat.created).toLocaleString('ja-JP')}`,
...
ただし、ユーザー入力を扱う Shell UI はブラウザのメインスレッドで動き、PID=2 のシェルプロセスとして `SysHelper` 経由でカーネルに操作を依頼します。`kill 2` で shell PCB が zombie になると、次の入力時に新しい shell PCB を fork して再起動します。
...
```typescript
getPageMap(): number[] {
  return Array.from(this.pageTable); // 0=空き, pid=使用中
}
```

**② `Syscall.ts` に `SYS.MEMMAP` を追加**
...
**③ `commands/index.ts` にコマンドを登録**

```typescript
commands.set('memmap', async (_args, _stdin, sys) => {
  const map = await sys.call(SYS.MEMMAP) as number[];
  const bar = map.map(pid => pid === 0 ? '[.]' : `[${pid}]`).join('');
  const used = map.filter(p => p !== 0).length;
  return [
    `Page map (${map.length} pages, 4KB each):`,
    bar,
    `Used: ${used} / ${map.length} pages (${used * 4}KB / ${map.length * 4}KB)`,
  ].join('\n');
});
```
...
| Module 6 | 自由課題 | ここまでの概念を組み合わせた拡張 |
